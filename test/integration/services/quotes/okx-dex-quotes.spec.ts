...
const describeIfCredentials = hasOkxCredentials ? describe : describe.skip;

describeIfCredentials('OKX DEX Quote Integration Tests', () => {
  describe('OKX DEX Quote Source', () => {
    when('requesting ETH to USDC quote on Ethereum', () => {
      ...
      given(async () => {
        const quotes = await quoteService.getAllQuotes({
          request: {
            ...
            filters: { includeSources: ['okx-dex'] },
          },
          config: {
            timeout: '30s',
            ignoredFailed: false,
      ...
      then('quote is returned successfully', () => {
        ...
        expect(quote.source.id).to.equal('okx-dex');
    ...
    when('requesting quote on Cronos', () => {
      let response: QuoteResponse | FailedResponse | undefined;

      given(async () => {
        const quotes = await quoteService.getAllQuotes({
          request: {
            chainId: Chains.CRONOS.chainId,
            sellToken: TOKENS.cronos.NATIVE,
            buyToken: TOKENS.cronos.USDC,
            order: {
              type: 'sell',
              sellAmount: parseEther('100'), // 100 CRO
            },
            slippagePercentage: 1,
            takerAddress: TEST_WALLET,
            filters: { includeSources: ['okx-dex'] },
      ...
      then('quote is returned for Cronos', () => {
        expect(response).to.exist;
        if (response && 'failed' in response) {
          console.log('Quote failed:', response.error);
          return;
        ...
        const quote = response as QuoteResponse;
        expect(quote.source.id).to.equal('okx-dex');
        expect(quote.chainId).to.equal(Chains.CRONOS.chainId);
      });
    });
  ...
  describe('OKX DEX Source Availability', () => {
    when('checking supported sources', () => {
      then('OKX DEX is available for Ethereum', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.ETHEREUM.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('okx-dex');
      });

      then('OKX DEX is available for Polygon', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.POLYGON.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('okx-dex');
      ...
      then('OKX DEX is available for Arbitrum', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.ARBITRUM.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('okx-dex');
      ...
      then('OKX DEX is available for Base', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.BASE.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('okx-dex');
      ...
      then('OKX DEX is available for BNB Chain', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
      ...
      then('OKX DEX is available for Avalanche', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
      ...
      then('OKX DEX is available for Cronos', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.CRONOS.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('okx-dex');
      ...
      then('OKX DEX is available for Sonic', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
  ...
  describe('Quote Response Structure', () => {
    when('receiving an OKX DEX quote', () => {
      ...
      then('has correct source information', () => {
        ...
        expect(quote.source.id).to.equal('okx-dex');
        expect(quote.source.name).to.equal('OKX Dex');
      });
