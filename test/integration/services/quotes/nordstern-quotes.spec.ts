...
const TOKENS = {
  ethereum: {
    ...
    DAI: '0x6B175474E89094C44Da98b954EesddFD691f40E0e',
  },
  polygon: {
    NATIVE: Addresses.NATIVE_TOKEN,
...
describe('Nordstern Quote Integration Tests', () => {
  describe('Nordstern Quote Source', () => {
    when('requesting ETH to USDC quote on Ethereum', () => {
      ...
      given(async () => {
        const quotes = await quoteService.getAllQuotes({
          request: {
            ...
            filters: { includeSources: ['nordstern'] },
          },
          config: {
            timeout: '30s',
            ignoredFailed: false,
      ...
      then('quote is returned successfully', () => {
        ...
        expect(quote.source.id).to.equal('nordstern');
      ...
      given(async () => {
        const quotes = await quoteService.getAllQuotes({
          request: {
            ...
            order: {
              type: 'sell',
              sellAmount: parseEther('10'), // 10 MATIC
            },
            slippagePercentage: 1,
            takerAddress: TEST_WALLET,
            filters: { includeSources: ['nordstern'] },
    ...
    when('requesting quote with invalid tokens', () => {
      let response: QuoteResponse | FailedResponse | undefined;
  ...
  describe('Nordstern Source Availability', () => {
    when('checking supported sources', () => {
      then('Nordstern is available for Ethereum', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.ETHEREUM.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('nordstern');
      });

      then('Nordstern is available for Polygon', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.POLYGON.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('nordstern');
      ...
      then('Nordstern is available for Arbitrum', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.ARBITRUM.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('nordstern');
      ...
      then('Nordstern is available for Base', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
          chainId: Chains.BASE.chainId,
        });
        expect(Object.keys(supportedSources)).to.include('nordstern');
      ...
      then('Nordstern is available for BNB Chain', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
      ...
      then('Nordstern is available for Avalanche', () => {
        const supportedSources = quoteService.supportedSourcesInChain({
  ...
  describe('Quote Response Structure', () => {
    when('receiving a Nordstern quote', () => {
      ...
      then('has correct source information', () => {
        ...
        expect(quote.source.id).to.equal('nordstern');
        expect(quote.source.name).to.equal('Nordstern');
      });
      ...
      then('has custom data with transaction info', () => {
        ...
        expect(quote.customData.tx.data).to.be.a('string');
      });
    });
  });
});
